...
class CCXTHyperliquidConnection:
    ...
    def place_market_order(self, asset: str, is_buy: bool, sz: float) -> Dict[str, Any]:
        ...
        try:
            # Obtener precio actual para registro
            price = self.get_market_price(symbol)
            
            # Colocar orden de mercado
            order = self.exchange.create_order(symbol, 'market', side, sz)
            
            logger.info(f"Orden de mercado colocada: {asset}, {'compra' if is_buy else 'venta'}, {sz}, precio aprox: {price}")
